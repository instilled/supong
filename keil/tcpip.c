...
#include "tcpip.h"
#include "ENET.h"         // Keil: Line added
...
// Keil: Timer 0 is used for TCP retransmission control
  ...
  TIM0->OC2R = 62875;                        // 262mSec
  TIM0->CR2  = TIM_FLAG_OC2 | (200 - 1);     // tclk = fpclk/200
  TIM0->CR1  = TIM_PWM | 0x8000;             // Enable Counter
  VIC0->VAiR[15] = (unsigned int)TCPClockHandler;
  VIC0->INTSR   &= ~(1<<4);
  VIC0->VCiR[15] = 0x20 | 4;                 // Set interrupt vector 15
  VIC0->INTER  |= (1<<4);

  SCU->GPIOANA = 0x03;                       // Enable AIN0, AIN1 pins
...
unsigned int IsBroadcast(void) {
  ...
  CopyFromFrame_ENET(&RecdDestMAC,  6);           // receive DA to see if it was a broadcast
  CopyFromFrame_ENET(&RecdFrameMAC, 6);           // store SA (for our answer)
    ...
    case LISTENING :
    {
      if (TCPFlags & TCP_ACTIVE_OPEN)            // stack has to open a connection?
        if (TCPFlags & IP_ADDR_RESOLVED)         // IP resolved?
          ...
          {
            TCPSeqNr = ((unsigned long)ISNGenHigh << 16) | (TIM0->CNTR & 0xFFFF);  // Keil: changed from TAR to TIM0->CNTR; 
                                                                // set local ISN
...
void ProcessEthBroadcastFrame(void)
{
  ...
  if (ReadFrameBE_ENET() == FRAME_ARP)           // get frame type, check for ARP
    if (ReadFrameBE_ENET() == HARDW_ETH10)       // Ethernet frame
      if (ReadFrameBE_ENET() == FRAME_IP)        // check protocol
        if (ReadFrameBE_ENET() == IP_HLEN_PLEN)  // check HLEN, PLEN
          if (ReadFrameBE_ENET() == OP_ARP_REQUEST)
          {
            DummyReadFrame_ENET(6);              // ignore sender's hardware address
            CopyFromFrame_ENET(&RecdFrameIP, 4); // read sender's protocol address
            DummyReadFrame_ENET(6);              // ignore target's hardware address
            CopyFromFrame_ENET(&TargetIP, 4);    // read target's protocol address
...
void ProcessEthIAFrame(void)
{
  ...
  switch (ReadFrameBE_ENET())                     // get frame type
  {
    case FRAME_ARP :                             // check for ARP
    {
      if ((TCPFlags & (TCP_ACTIVE_OPEN | IP_ADDR_RESOLVED)) == TCP_ACTIVE_OPEN)
        if (ReadFrameBE_ENET() == HARDW_ETH10)         // check for the right prot. etc.
          if (ReadFrameBE_ENET() == FRAME_IP)
            if (ReadFrameBE_ENET() == IP_HLEN_PLEN)
              if (ReadFrameBE_ENET() == OP_ARP_ANSWER)
              {
                TCPStopTimer();                       // OK, now we've the MAC we wanted ;-)
                CopyFromFrame_ENET(&RemoteMAC, 6);    // extract opponents MAC
    ...
    {
      if ((ReadFrameBE_ENET() & 0xFF00 ) == IP_VER_IHL)     // IPv4, IHL=5 (20 Bytes Header)
      {                                                    // ignore Type Of Service
        RecdIPFrameLength = ReadFrameBE_ENET();             // get IP frame's length
        ReadFrameBE_ENET();                                 // ignore identification

        if (!(ReadFrameBE_ENET() & (IP_FLAG_MOREFRAG | IP_FRAGOFS_MASK)))  // only unfragm. frames
        {
          ProtocolType = ReadFrameBE_ENET() & 0xFF;         // get protocol, ignore TTL
          ReadFrameBE_ENET();                               // ignore checksum
          CopyFromFrame_ENET(&RecdFrameIP, 4);              // get source IP
          CopyFromFrame_ENET(&TargetIP, 4);                 // get destination IP
...
void ProcessICMPFrame(void)
{
  ...
  ICMPTypeAndCode = ReadFrameBE_ENET();           // get Message Type and Code
  ReadFrameBE_ENET();                             // ignore ICMP checksum
...
void ProcessTCPFrame(void)
{
  ...
  TCPSegSourcePort = ReadFrameBE_ENET();                    // get ports
  TCPSegDestPort = ReadFrameBE_ENET();

  if (TCPSegDestPort != TCPLocalPort) return;              // drop segment if port doesn't match

  TCPSegSeq = (unsigned long)ReadFrameBE_ENET() << 16;      // get segment sequence nr.
  TCPSegSeq |= ReadFrameBE_ENET();

  TCPSegAck = (unsigned long)ReadFrameBE_ENET() << 16;      // get segment acknowledge nr.
  TCPSegAck |= ReadFrameBE_ENET();

  TCPCode = ReadFrameBE_ENET();                             // get control bits, header length...
  ...
  if (TCPHeaderSize > TCP_HEADER_SIZE)                     // ignore options if any
    DummyReadFrame_ENET(TCPHeaderSize - TCP_HEADER_SIZE);
    ...
    default :
    {
      ...
      if ((TCPStateMachine == ESTABLISHED) || (TCPStateMachine == FIN_WAIT_1) || (TCPStateMachine == FIN_WAIT_2))
        if (NrOfDataBytes)                                 // data available?
          ...
          {
            DummyReadFrame_ENET(6);                        // ignore window, checksum, urgent pointer
            CopyFromFrame_ENET(RxTCPBuffer, NrOfDataBytes);// fetch data and
...
void PrepareICMP_ECHO_REPLY(void)
{
  ...
  *(unsigned short *)&TxFrame2[ICMP_CHKSUM_OFS] = 0;                   // initialize checksum field
  ...
  CopyFromFrame_ENET(&TxFrame2[ICMP_DATA_OFS], ICMPDataCount);        // get data to echo...
...
void TCPClockHandler(void) __irq                // Keil: interrupt service routine for timer 0
{
  ...
  TIM0->SR  = ~TIM_FLAG_OC2;                     // Clear interrupt flag
...
{
  CopyToFrame_ENET(TxFrame1, TxFrame1Size);
}
...
{
  CopyToFrame_ENET(TxFrame2, TxFrame2Size);
}
